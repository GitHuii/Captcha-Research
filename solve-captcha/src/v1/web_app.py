...
class CaptchaPredictor:
    def __init__(self, model_path, alphabet_path):
        ...
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Không tìm thấy file model trọng số tại: {model_path}.\nHãy đảm bảo bạn đã chép file mô hình tải từ Google Colab vào thư mục solve-captcha/model/v1/")
...
def main():
    # Đi lên 3 cấp từ solve-captcha/src/v1/web_app.py để ra thư mục gốc solve-captcha
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    model_path = os.path.join(base_dir, "model", "v1", "captcha_model.pth")
    alphabet_path = os.path.join(base_dir, "model", "v1", "alphabet.json")
    ...
    try:
        ...
        print("Mẹo: Bạn đã copy file 'captcha_model.pth' và 'alphabet.json' vào thư mục 'solve-captcha/model/v1/' chưa?")
        return
